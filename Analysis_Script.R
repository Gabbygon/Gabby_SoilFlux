...
soil_temp15 |>
  ...
  geom_line() +
  geom_point() +
  labs(title = "Soil Temperature by Sensor ", x = "Month", y = "Average Temp")
...
ggplot(
  ...
  geom_boxplot() +
  # Separate by month to be able to graphs each month into its individual section
  facet_wrap(~month, scales = "free_x", ncol = 6) +
...
# Box plots acrosss each months with points from moisture
soil_vwc15 |>
  slice_sample(n = 1000) |>
  ggplot(
    aes(x = month, y = Value)
  ...
  geom_boxplot() +
  geom_jitter(pch = 19, width = 0.2, aes(color = month)) +
  facet_wrap(~month, scales = "free_x", ncol = 6) +
  labs(title = "Moisture by Month", y = "Temperature", x = "Month") +
  theme_minimal() +
...
# Randomly sub_sample the data to make plotting faster
f_soil_EC_15 |>
  # this data set is VERY large so just select 1000 random rows to plot
  slice_sample(n = 1000) |>
  ggplot(aes(x = TIMESTAMP, y = Value, color = Plot)) +
  geom_point() +
  # add a 'smoother' -- a smooth line that follows the data to help
...
f_soil_EC_15 |>
  ...
  geom_line() +
  geom_point() +
  labs(title = "Soil EC by Sensor ", x = "Month", y = "Average EC")
...
ggplot(
  ...
  geom_boxplot() +
  # Use to separate each month
  facet_wrap(~month, scales = "free_x", ncol = 6) +
  labs(title = "EC by Month", y = "EC", x = "Month") +
  theme_minimal()

# Box plot of EC by month across the year
f_soil_EC_15 |>
...
teros_combined |>
  ...
  geom_line(aes(y = Value_vwc15 * 70, color = "VWC")) + # re-scaled
...
teros_combined |>
  ...
  ggplot(aes(x = TIMESTAMP)) +
  geom_line(aes(y = Value_temp15, color = "Temperature")) +
  geom_line(aes(y = Value_vwc15 * 50, color = "VWC")) +
...
teros_combined |>
  ...
  ggplot(aes(x = TIMESTAMP)) +
  geom_line(aes(y = Value_temp15, color = "Temperature")) +
  geom_line(aes(y = Value_ec15 / 10, color = "EC")) +
  theme(axis.text.x = element_text(angle = 45, hjust = 1, vjust = 1)) +
  annotate("rect", xmin = flood_start, xmax = flood_end, ymin = -Inf, ymax = Inf, alpha = 0.1, fill = "green") +
  # separating by location
  facet_wrap(~Location) +
...
teros_combined |>
  group_by(Location) |>
  summarise(mean_temp5 = mean(Value_temp15, na.rm = TRUE)) |>
  separate(Location, into = c("grid_letter", "grid_number"), sep = 1) |>
